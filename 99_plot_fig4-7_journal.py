...
MODEL_FILES = [
    ...
    ("Hybrid(Ours)",   "predictions_test_RESCTX_OOF_pred_core_tfmr.csv"),
]
...
STYLE_MAP = {
    ...
    "Hybrid(Ours)":    {"c": "#D32F2F", "ls": "-",  "mk": "*", "lw": 2.5, "z": 4}, # 红色实线 (主角)
}
...
def main():
   ...
   # 可选：检查字体是否存在（不存在就打印出来方便你装/改名）
    ...
    ax1.set_title("(a) Error Analysis: Mean Absolute Error Across Aging Stages", fontweight='bold', pad=10)
    ax1.grid(False)
    ...
    ax2.set_ylabel("Bias (Prediction Bias)", fontweight='bold')
    ax2.set_xlabel("True SOH (Aging Stage)", fontweight='bold')
    ax2.set_title("(b) Calibration Analysis: Bias Across Aging Stages", fontweight='bold', pad=10)
    ax2.grid(False)
    
    # 标注长尾改进 (找出 Hybrid(Ours) 和 HGBR 在最左侧的差距)
    if "Hybrid(Ours)" in data_map and "HGBR" in data_map:
        df_base = data_map["HGBR"]
        df_best = data_map["Hybrid(Ours)"]
        # 找最小的 x (Low SOH)
    ...
    plt.tight_layout()
    out_path = os.path.join(OUT_DIR, "Fig4-7_MultiModel_SoH_Comparison.png")
