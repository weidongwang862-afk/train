...
def plot_data_heterogeneity_corrected():
    ...
    # --- 子图 A: 样本规模 (Scale) ---
    ax1.bar(x, df_agg["count"], color='#3F51B5', width=bar_width, alpha=0.9, label='Sample')
    ax1.set_ylabel("Sample", fontweight='bold')
    # 修正标题为中文，符合图 3-1 的定位
    ax1.set_title("(a) Sample Size Distribution", fontweight='bold', pad=10, loc='left')
    ...
    # --- 子图 B: 里程跨度 (Coverage) ---
    ax2.bar(x, df_agg["odo_span"], color='#009688', width=bar_width, alpha=0.9, label='Mileage Coverage')
    ax2.set_ylabel("Mileage Coverage (10⁴ km)", fontweight='bold')
    ax2.set_title("(b) Mileage Coverage", fontweight='bold', pad=10, loc='left')
    ...
    # =========================
    ax3.set_title("(c) Aging Heterogeneity", loc='left', fontweight='bold')
    ...
    ax3.set_ylabel("SoH (Normalized)", fontweight='bold')
    ax3.set_xlabel("Vehicle ID (Sorted by Sample Size)", fontweight='bold')
