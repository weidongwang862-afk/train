...
PRED_DEEP_CSV = r"E:\RAW_DATA\outputs\09_seq_featcore\09_residual\predictions_test_RESCTX_OOF_pred_core_tfmr.csv"  # 你也可以换成 ATTENTION/GRU 等
SEGMENT_SUMMARY_PARQUET = rf"E:\RAW_DATA\outputs\02_segments\segment_summary_{VIN_TARGET}.parquet"  # 没有就放空字符串 ""

OUT_FIG = rf"E:\RAW_DATA\Fig4-4_case_{VIN_TARGET}.png"

# Deep 模型名显示（用于图例）
DEEP_NAME = "Hybrid(Transformer)"
...
def load_pred_csv(path, model_name):
    ...
    y_pred_col = pick_col(df, ["y_pred", "final_pred", "pred_core", "prediction", "y_hat", "soh_pred", "SoH_pred"],
...
def plot_case(vin_target, df_hgbr, df_deep, seg_df=None, out_path=None):
    ...
    ax1.plot(x, y_pred_h, lw=2.0, ls="--", color="#6c757d", label="HGBR (Baseline)")
    ax1.plot(x, y_pred_d, lw=2.2, color="#d62728", label=DEEP_NAME)

    ax1.set_ylabel("SoH / Battery Capacity (Normalized)", fontsize=13)
    ax1.grid(False)
    ax1.legend(loc="upper right", frameon=True, fontsize=10)

    # 下图：Residual
    ax2.axhline(0.0, color="#222222", lw=1.2, alpha=0.8)
    ax2.plot(x, res_h, lw=1.6, ls="--", color="#6c757d", label="HGBR Residual")
    ax2.plot(x, res_d, lw=1.8, color="#d62728", label=f"{DEEP_NAME} Residual")
    ...
    # 标题
    fig.suptitle(f"Worst VIN Analysis (Example: VIN125): {vin_target}", fontsize=15, y=0.98)
