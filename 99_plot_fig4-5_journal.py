   ...
   # 可选：检查字体是否存在（不存在就打印出来方便你装/改名）
    ...
    if len(segs_in_window) > 0:
        ...
        if len(chg) > 0:
            ...
            ax1.text((xs + xe)/2, ax1.get_ylim()[1], "Label window (for capacity calculation)",
    ...
    # 4.2 绘制波形 (心电图)
    # 左轴：电流 (蓝色)
    ln1 = ax1.plot(x_time, y_current_plot, color='#1565C0', linewidth=1.2, label='Current (A)', zorder=2)
    ax1.set_xlabel("Times (Minutes)", fontweight='bold')
    ax1.set_ylabel("Current (A)", fontweight='bold', color='#1565C0')
    ...
    # 右轴：车速 (灰色虚线)
    ax2 = ax1.twinx()
    ln2 = ax2.plot(x_time, y_speed_plot, color='#78909C', linewidth=0.9, linestyle='--', alpha=0.55, label='Speed (km/h)', zorder=1)
    ax2.set_ylabel("Speed (km/h)", fontweight='bold', color='#78909C')
    ax2.tick_params(axis='y', labelcolor='#78909C',colors="0.45")

    # 4.3 装饰
    ax1.set_title("Segmentation and Event Window Definition Illustration", fontweight='bold', fontsize=16, pad=15)
    
    # 构建图例
    # 色块图例
    patches = [
        mpatches.Patch(color=color_map['charge'], label='Charge'),
        mpatches.Patch(color=color_map['drive'], label='Drive'),
        mpatches.Patch(color=color_map['rest'], label='Rest'),
    ]
    # 线条图例 (合并 ln1 和 ln2)
    lines = [
        plt.Line2D([0], [0], color='#1565C0', lw=1.5, label='Current (A)'),
        plt.Line2D([0], [0], color='#78909C', lw=1.5, ls='--', label='Speed (km/h)')
    ]
    
    ax1.legend(handles=patches + lines, loc='upper center', bbox_to_anchor=(0.5, -0.15),
               frameon=False, ncol=5, fontsize=11)
    win_patch = mpatches.Patch(color="#90CAF9", alpha=0.12, label="Label window (for capacity calculation)")
    handles = patches + [win_patch] + lines
    
    plt.tight_layout()
    save_path = 'Fig3-2_Segmentation_ECG.png'
